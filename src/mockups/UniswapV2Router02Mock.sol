...
contract UniswapV2Router02Mock is Test {
    ...
    function swapExactTokensForTokens(
        ...
        //Cheat balance of
        uint256 slot = stdstore.target(address(path[0])).sig(IERC20(path[0]).balanceOf.selector).with_key(address(msg.sender)).find();
        ...
        //Cheat balance of
        uint256 slot2 = stdstore.target(address(path[1])).sig(IERC20(path[1]).balanceOf.selector).with_key(address(to)).find();
