...
contract UniswapV2SubRegistry is SubRegistry {
  ...
  struct AssetInformation {
    address pair;   // Contract address of the Uniswap V2 Liquidity pool
    address token0; // Contract address of token0
    address token1; // Contract address of token1
  }
   ...
   */
    function getReservesAfterArbitrage(
        ...
        uint256 trustedPriceToken0,
        uint256 trustedPriceToken1
    ) view internal returns (uint256 reserve0, uint256 reserve1) {
   ...
   */
    function computeProfitMaximizingTrade(
        ...
        uint256 reserve0,
        uint256 reserve1
    ) pure internal returns (bool token0ToToken1, uint256 amountIn) {
