...
contract Vault {
    ...
    modifier onlyFactory() {
        require(msg.sender == IMainRegistry(_registryAddress).factoryAddress(), "VL: You are not the factory");
        _;
    }
     ...
     */
    // TODO:
    function initialize(address _owner, address registryAddress, uint16 _vaultVersion) external payable {
        require(vaultVersion == 0, "V_I: Already initialized!");
        require(_vaultVersion != 0, "V_I: Invalid vault version");
        _registryAddress = registryAddress;
        owner = _owner;
        vault._liquidityThreshold = 110;
        vault._collateralFactor = 150;
        ...
        onlyOwner
    {
        ...
        require(
            IRegistry(_registryAddress).batchIsWhiteListed(assetAddresses, assetIds), "Not all assets are whitelisted!"
        );
    ...
    function _depositERC20(address _from, address ERC20Address, uint256 amount) private {
        ...
        _erc20Stored.push(ERC20Address);
    ...
    function _depositERC721(address _from, address ERC721Address, uint256 id) private {
        ...
        _erc721Stored.push(ERC721Address);
        //TODO: see what the most gas efficient manner is to store/read/loop over this list to avoid duplicates
    ...
    function _depositERC1155(address _from, address ERC1155Address, uint256 id, uint256 amount) private {
        ...
        if (!addrSeen) {
            _erc1155Stored.push(ERC1155Address);
            //TODO: see what the most gas efficient manner is to store/read/loop over this list to avoid duplicates
        ...
        returns (address[] memory assetAddresses, uint256[] memory assetIds, uint256[] memory assetAmounts)
    {
        uint256 totalLength;
        unchecked {
            totalLength = _erc20Stored.length + _erc721Stored.length + _erc1155Stored.length;
     ...
     */
    function getValue(uint8 baseCurrency) public view returns (uint256 vaultValue) {
        (address[] memory assetAddresses, uint256[] memory assetIds, uint256[] memory assetAmounts) =
            generateAssetData();
        vaultValue = IRegistry(_registryAddress).getTotalValue(assetAddresses, assetIds, assetAmounts, baseCurrency);
    }
     ...
     */
    function _setBaseCurrency(uint256 newBaseCurrency) private {
        require(getUsedMargin() == 0, "VL: Can't change baseCurrency when openDebt > 0");
        require(
            newBaseCurrency + 1 <= IMainRegistry(_registryAddress).baseCurrencyCounter(), "VL: baseCurrency not found"
        );
        vault._baseCurrency = uint8(newBaseCurrency);
        //Change this to where ever it is going to be actually set
    }
    // TODO: Move to library and use it in the risk module total value calculation - 14/09/22 - Zeki
    ...
    function sumElementsOfList(uint256[] memory _data) public payable returns (uint256 sum) {
        //cache
    ...
    function getUsedMargin() public returns (uint128 usedMargin) {
        ...
        usedMargin = uint128(IERC4626(_debtToken).maxWithdraw(address(this)));
     ...
     */
    function increaseMarginPosition(uint256 baseCurrency, uint256 amount)
        public
        onlyAuthorized
        returns (bool success)
    {
        if (baseCurrency != vault._baseCurrency) {
