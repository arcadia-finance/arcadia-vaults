...
contract Vault {
  ...
  */
    function withdraw(
        ...
        if (openDebt != 0) {
            require(
                ((getValue(debt._numeraire) * 100) / openDebt) >
                    debt._collThres,
                "Cannot withdraw since the collateral value would become too low!"
            );
        }
    }
  ...
  */
    function getValueOfAssets(
        ...
        uint256[] memory assetIds,
        uint256[] memory assetAmounts
    ) public view returns (uint256 vaultValue) {
