    ...
    if (minCollValue == 0) {
      ...
      for (uint256 i = 1; i < valuesPerCreditRatingLength;) {
        ...
        if (totalValue + value < minCollValue) {
          collateralInterestRate += creditRatingToInterestRate[i].mulDivDown(value, minCollValue);
          totalValue += value;
        } else {
      ...
      value = minCollValue - totalValue;
      collateralInterestRate += creditRatingToInterestRate[0].mulDivDown(value, minCollValue);
