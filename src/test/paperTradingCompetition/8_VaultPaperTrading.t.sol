...
contract VaultPaperTradingInheritedTest is vaultTests {
  ...
  constructor() vaultTests() {
    ...
    stableEth = new StablePaperTrading("Arcadia ETH Stable Mock", "masETH", uint8(Constants.stableEthDecimals), 0x0000000000000000000000000000000000000000, address(factoryContr));
  ...
  function setUp() public override {
    ...
    vm.startPrank(vaultOwner);
    vault.initialize(vaultOwner, address(mainRegistry), address(stableUsd), address(stakeContract), address(interestRateModule), address(tokenShop));
    vm.stopPrank();
  }

  //input as uint8 to prevent too long lists as fuzz input
  function testShouldFailIfLengthOfListDoesNotMatch(uint8 addrLen, uint8 idLen, uint8 amountLen, uint8 typesLen) public override {

  }

  function testShouldFailIfERC20IsNotWhitelisted(address inputAddr) public override {

  }

  function testShouldFailIfERC721IsNotWhitelisted(address inputAddr, uint256 id) public override {
  ...
  function testDepositERC20ERC721(uint8 erc20Amount1, uint8 erc20Amount2) public override {

  }


    function testDepositERC20ERC721ERC1155(uint8 erc20Amount1, uint8 erc20Amount2, uint8 erc1155Amount) public override {
  ...
  function testNotAllowWithdrawERC20fterTakingCredit (uint8 baseAmountDeposit, uint24 baseAmountCredit, uint8 baseAmountWithdraw) public override {

  }

  function testWithrawERC721AfterTakingCredit(uint128[] calldata tokenIdsDeposit, uint8 baseAmountCredit) public override {

  }

  function testNotAllowERC721Withdraw(uint128[] calldata tokenIdsDeposit, uint8 amountsWithdrawn) public override {

  }

  function testNotAllowedToWithdrawnByNonOwner(uint8 depositAmount, uint8 withdrawalAmount, address sender) public override {
  ...
  function testGetValueGasUsage(uint8 depositAmount, uint128[] calldata tokenIds) public override {
  ...
  function testGetRemainingCreditAfterTopUp(uint8 amountEth, uint8 amountLink, uint128[] calldata tokenIds) public override {

  }

  function testGetRemainingCreditAfterTakingCredit(uint8 amountEth, uint128 amountCredit) public override {
  ...
  function testTakeCreditAsNonOwner(uint8, uint128 amountCredit) public override {
    (,uint16 _collThres,,,,) = vault.debt();
  ...
  function testSyncDebtUnchecked(uint64 base, uint24 deltaBlocks, uint128 openDebt) public override {
  ...
  function testRemainingCreditUnchecked(uint128 amountEth, uint8 factor) public override {

  }

  function testTransferOwnershipOfVaultByNonOwner(address sender) public override {
  ...
  function testCreateEthVault() public {
    ...
    vault.initialize(vaultOwner, address(mainRegistry), address(stableEth), address(stakeContract), address(interestRateModule), address(tokenShop));
    vm.stopPrank();
