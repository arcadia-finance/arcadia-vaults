...
contract LiquidatorPaperTradingInheritedTest is LiquidatorTest {
  ...
  constructor () LiquidatorTest() {
    ...
    snx = new ERC20PaperTrading("SNX Mock", "mSNX", uint8(Constants.snxDecimals), address(tokenShop));
    link = new ERC20PaperTrading("LINK Mock", "mLINK", uint8(Constants.linkDecimals), address(tokenShop));
    bayc = new ERC721PaperTrading("BAYC Mock", "mBAYC", address(tokenShop));
    wbayc = new ERC20PaperTrading("wBAYC Mock", "mwBAYC", uint8(Constants.wbaycDecimals), address(tokenShop));
    ...
    floorERC1155SubRegistry.setAssetInformation(FloorERC1155SubRegistry.AssetInformation({oracleAddresses: oracleInterleaveToEthEthToUsd, id:0, assetAddress: address(interleave)}), emptyList);
  ...
  function setUp() public override {
    ...
    proxyAddr2 = factory.createVault(uint256(keccak256(abi.encodeWithSignature("doRandom(uint256,uint256,bytes32)", block.timestamp, block.number, blockhash(block.number))))+1, Constants.UsdNumeraire);
  ...
  function testNotAllowAuctionHealthyVault(uint128, uint128 amountCredit) public override {
  ...
  function testShowVaultAuctionPrice(uint128, uint256 newPrice) public override {
    (, uint16 collThresProxy, uint8 liqThresProxy,,,) = proxy.debt();
    vm.assume(newPrice/ liqThresProxy  < rateEthToUsd / collThresProxy);
  ...
  function testAuctionPriceDecrease(uint128, uint256, uint64) public override {

  }

  function testBuyVault(uint128, uint256, uint64) public override {

  }

  function testWithrawAssetsFromPurchasedVault(uint128, uint256, uint64) public override {
  ...
  function testClaimSingleWrongLife(uint128, uint16, uint16) public override {
  ...
  function buyEthWithLoan(address owner, Vault proxyContract) public {
    ...
    uint256 ethAmount = 10 ** Constants.ethDecimals * usdAmount * 10 ** Constants.oracleEthToUsdDecimals / Constants.WAD  / rateEthToUsd;
