...
//this is a before
  ...
  function testOwnerSetsCollateralInterestRates (uint64 interestRate) public {
    ...
    assertEq(interestRate, interestRateModule.creditRatingToInterestRate(0));
    assertEq(interestRate, interestRateModule.creditRatingToInterestRate(1));
  }  

  function testReturnBaseInterestRateWhenNoMinimalCollateralValueIsGiven (uint64 baseInterestRate, uint256 value) public {
    vm.startPrank(creatorAddress);
    interestRateModule.setBaseInterestRate(baseInterestRate);
    vm.stopPrank();
    ...
    assertEq(baseInterestRate, interestRateModule.getYearlyInterestRate(valuesPerCreditRating, 0));
  }

  function testReturnBaseRateWhenNoValuesAreGiven (uint64 baseInterestRate, uint256 minCollValue) public {
    vm.startPrank(creatorAddress);
    interestRateModule.setBaseInterestRate(baseInterestRate);
    vm.stopPrank();
    ...
    assertEq(baseInterestRate, interestRateModule.getYearlyInterestRate(valuesPerCreditRating, minCollValue));
  }

  function testReturnCollateralInterestRateWithoutCategory0 (uint128 cat1Value, uint128 cat2Value, uint128 minCollValue) public {
    ...
    uint256 expectedInterestRate = (cat1Value * interestRates[1]) / minCollValue + ((minCollValue - cat1Value) * interestRates[2]) / minCollValue;

    uint256[] memory valuesPerCreditRating = new uint256[](3);
    valuesPerCreditRating[1] = cat1Value;
    valuesPerCreditRating[2] = cat2Value;

    uint256 actualInterestRate = interestRateModule.getYearlyInterestRate(valuesPerCreditRating, minCollValue);

    assertEq(expectedInterestRate, actualInterestRate);
  }

  function testReturnCollateralInterestRateWithCategory0 (uint128 cat0Value, uint128 cat1Value, uint128 minCollValue) public {
    ...
    uint256 expectedInterestRate = (cat1Value * interestRates[1]) / minCollValue + ((minCollValue - cat1Value) * interestRates[0]) / minCollValue;
  ...
  function testReturnCollateralInterestRateWithTotalValueLessThenMinimalCollateralValue (uint128 cat1Value, uint128 minCollValue) public {
    vm.assume(cat1Value < minCollValue);
  ...
  function testReturnSumOfBaseAndCollateralInterestRates (uint64 baseInterestRate, uint64 collateralInterestRate,  uint128 cat1Value, uint128 minCollValue) public {
    vm.assume(cat1Value > minCollValue);
    vm.assume(minCollValue > 0);
    vm.assume(collateralInterestRate > 0);
    vm.assume(cat1Value < type(uint128).max / uint128(collateralInterestRate));
    ...
    uint256 expectedInterestRate = uint256(baseInterestRate) + uint256(collateralInterestRate);
