...
contract MainRegistryTest is DSTest {
  ...
  function testOwnerAddsNumeraireWithNonExistingCreditRatingCategory () public {
    vm.startPrank(creatorAddress);
    mainRegistry.addSubRegistry(address(standardERC20Registry));
    standardERC20Registry.setAssetInformation(StandardERC20Registry.AssetInformation({oracleAddresses: oracleEthToUsdArr, assetUnit: uint64(10**Constants.ethDecimals), assetAddress: address(eth)}), emptyList);
    standardERC20Registry.setAssetInformation(StandardERC20Registry.AssetInformation({oracleAddresses: oracleLinkToUsdArr, assetUnit: uint64(10**Constants.linkDecimals), assetAddress: address(link)}), emptyList);
  ...
  function testGetWhitelistWithMultipleAssets () public {
    ...
    mainRegistry.addSubRegistry(address(floorERC721SubRegistry));
    standardERC20Registry.setAssetInformation(StandardERC20Registry.AssetInformation({oracleAddresses: oracleEthToUsdArr, assetUnit: uint64(10**Constants.ethDecimals), assetAddress: address(eth)}), emptyList);
    standardERC20Registry.setAssetInformation(StandardERC20Registry.AssetInformation({oracleAddresses: oracleSnxToEthEthToUsd, assetUnit: uint64(10**Constants.snxDecimals), assetAddress: address(snx)}), emptyList);
    floorERC721SubRegistry.setAssetInformation(FloorERC721SubRegistry.AssetInformation({oracleAddresses: oracleWbaycToEthEthToUsd, idRangeStart:0, idRangeEnd:9999, assetAddress: address(bayc)}), emptyList);
    vm.stopPrank();
  ...
  function testGetTotalValueCalculateValueInNumeraireFromValueInUsdSucces (uint256 rateEthToUsdNew, uint256 amountLink, uint8 linkDecimals) public {
    vm.assume(linkDecimals <= 18);
    vm.assume(rateEthToUsdNew <= uint256(type(int256).max));
    vm.assume(rateEthToUsdNew > 0);
    vm.assume(amountLink <= type(uint256).max / uint256(rateLinkToUsd) / Constants.WAD / 10 ** (Constants.oracleEthToUsdDecimals - Constants.oracleLinkToUsdDecimals));
    ...
    uint256 actualTotalValue = mainRegistry.getTotalValue(assetAddresses, assetIds, assetAmounts, Constants.EthNumeraire);

    uint256 linkValueInUsd = assetAmounts[0] * rateLinkToUsd * Constants.WAD / 10 ** Constants.oracleLinkToUsdDecimals / 10 ** linkDecimals;
  ...
  function testGetTotalValueCalculateValueInNumeraireFromValueInUsdOverflow (uint256 rateEthToUsdNew, uint256 amountLink, uint8 linkDecimals) public {
    ...
    mainRegistry.getTotalValue(assetAddresses, assetIds, assetAmounts, Constants.EthNumeraire);
  }

  function testGetTotalValueCalculateValueInNumeraireFromValueInUsdWithRateZero (uint256 amountLink) public {
  ...
  function testGetTotalValueSucces () public {
    ...
    uint256 ethValueInEth = Constants.WAD * assetAmounts[0] / 10 ** Constants.ethDecimals;
    uint256 linkValueInUsd = Constants.WAD * rateLinkToUsd * assetAmounts[1] / 10 ** (Constants.oracleLinkToUsdDecimals + Constants.linkDecimals);
    uint256 linkValueInEth = linkValueInUsd * 10 ** Constants.oracleEthToUsdDecimals / rateEthToUsd;
    uint256 baycValueInEth = Constants.WAD * rateWbaycToEth * assetAmounts[2] / 10 ** Constants.oracleWbaycToEthDecimals;
  ...
  function testGetListOfValuesPerAssetSucces () public {
    ...
    uint256[] memory actualListOfValuesPerAsset = mainRegistry.getListOfValuesPerAsset(assetAddresses, assetIds, assetAmounts, Constants.EthNumeraire);
  ...
  function testGetListOfValuesPerCreditRatingSucces () public {
    ...
    uint256[] memory actualListOfValuesPerCreditRating = mainRegistry.getListOfValuesPerCreditRating(assetAddresses, assetIds, assetAmounts, Constants.EthNumeraire);
    ...
    uint256[] memory expectedListOfValuesPerCreditRating = new uint256[](mainRegistry.CREDIT_RATING_CATOGERIES());
  ...
  function testOwnerSetsCreditRatingsSucces () public {
    ...
    assertEq(Constants.ethCreditRatingUsd, mainRegistry.assetToNumeraireToCreditRating(address(eth), Constants.UsdNumeraire));
    assertEq(Constants.ethCreditRatingEth, mainRegistry.assetToNumeraireToCreditRating(address(eth), Constants.EthNumeraire));
  }
