...
contract gasVaultAuction_2ERC202ERC721 is DSTest {
  ...
  function setUp() public {
    ...
    vm.prank(vaultOwner);
    uint256 valueEth = 10 ** 18 * rateEthToUsd / 10 ** Constants.oracleEthToUsdDecimals * s_3[0] / 10 ** Constants.ethDecimals;
    uint256 valueLink = 10 ** 18 * rateLinkToUsd / 10 ** Constants.oracleLinkToUsdDecimals * s_3[1] / 10 ** Constants.linkDecimals;
    uint256 valueBayc = 10 ** 18 * rateWbaycToEth * rateEthToUsd / 10 ** (Constants.oracleWbaycToEthDecimals + Constants.oracleEthToUsdDecimals) * s_3[2];
    uint256 valueMayc = 10 ** 18 * rateWmaycToUsd / 10 ** Constants.oracleWmaycToUsdDecimals * s_3[3];   
    proxy.takeCredit(uint128((valueEth + valueLink + valueBayc + valueMayc) * 100 / 150));

    vm.startPrank(oracleOwner);
    oracleEthToUsd.setAnswer(int256(rateEthToUsd)/2);
    oracleWbaycToEth.setAnswer(int256(rateWbaycToEth)/2);
    oracleLinkToUsd.setAnswer(int256(rateLinkToUsd)/2);
    oracleWmaycToUsd.setAnswer(int256(rateWmaycToUsd)/2);
    vm.stopPrank();
