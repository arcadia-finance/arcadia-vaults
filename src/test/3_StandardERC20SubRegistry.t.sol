...
//this is a before
  ...
  function testReturnUsdValueWhenNumeraireIsUsd (uint128 amountEth) public {
    ...
    uint256 expectedValueInUsd = amountEth * rateEthToUsd * Constants.WAD / 10 ** (Constants.oracleEthToUsdDecimals + Constants.ethDecimals);
    uint256 expectedValueInNumeraire = 0;

    SubRegistry.GetValueInput memory getValueInput = SubRegistry.GetValueInput({assetAddress:address(eth), assetId:0, assetAmount:amountEth, numeraire:0});
    (uint256 actualValueInUsd, uint256 actualValueInNumeraire) = standardERC20Registry.getValue(getValueInput);

    assertEq(actualValueInUsd, expectedValueInUsd);
    assertEq(actualValueInNumeraire, expectedValueInNumeraire);    
  }

  function testreturnNumeraireValueWhenNumeraireIsNotUsd (uint128 amountSnx) public {
    ...
    uint256 expectedValueInNumeraire = amountSnx * rateSnxToEth * Constants.WAD / 10 ** (Constants.oracleSnxToEthDecimals + Constants.snxDecimals);

    SubRegistry.GetValueInput memory getValueInput = SubRegistry.GetValueInput({assetAddress:address(snx), assetId:0, assetAmount:amountSnx, numeraire:1});
    (uint256 actualValueInUsd, uint256 actualValueInNumeraire) = standardERC20Registry.getValue(getValueInput);
  ...
  function testReturnUsdValueWhenNumeraireIsNotUsd (uint128 amountLink) public {
    //Does not test on overflow, test to check if function correctly returns value in Numeraire
    vm.startPrank(creatorAddress);
    ...
    uint256 expectedValueInUsd = amountLink * rateLinkToUsd * Constants.WAD / 10 ** (Constants.oracleLinkToUsdDecimals + Constants.linkDecimals);
    uint256 expectedValueInNumeraire = 0;

    SubRegistry.GetValueInput memory getValueInput = SubRegistry.GetValueInput({assetAddress:address(link), assetId:0, assetAmount:amountLink, numeraire:1});
    (uint256 actualValueInUsd, uint256 actualValueInNumeraire) = standardERC20Registry.getValue(getValueInput);
  ...
  function testReturnValueSucces (uint256 rateEthToUsdNew, uint256 amountEth) public {
    ...
    uint256 expectedValueInUsd =  Constants.WAD * rateEthToUsdNew / 10 ** Constants.oracleEthToUsdDecimals * amountEth / 10 ** Constants.ethDecimals;
    uint256 expectedValueInNumeraire = 0;
